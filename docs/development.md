...
Milestone B1 now adds static plate kinematics: connected seeded ownership, angular velocities, and relative motion at actual shared-boundary segments. Both views provide plate identity, boundary motion, and speed layers. Read [Plate kinematics](tectonics.md) for the current `tectonics-1` recipe and protocol 2 extension. No crust type, uplift, or geological time integration is implemented yet.
...
`surface-1` and `surface-rust-1` are not silently migrated. To explore an old seed under the new model, create a new recipe from its visible parameters and keep the old file unchanged; identical checksums are not promised. The old implementation remains available through Git history.
