...
Milestone B1 adds static plate kinematics: connected seeded ownership, angular velocities, and relative motion at actual shared-boundary segments. B2 adds independent continentality, approximate crust thickness/density, area fitting, and two crust layers. Read [Plate kinematics](tectonics.md) and [Initial crust](crust.md) for the current `crust-1` recipe and protocol 3 extension. No uplift, elevation, water, or geological time integration is implemented yet.
...
`surface-1`, `surface-rust-1`, and `tectonics-1` are not silently migrated. To explore an old seed under the new model, create a new recipe from its visible parameters and keep the old file unchanged; identical checksums are not promised. The old implementation remains available through Git history.
