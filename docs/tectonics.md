...
Status: implemented. This document records B1 with model recipe `tectonics-1`, binary protocol 2. [B2 initial crust](crust.md), [B3 elevation](terrain.md), [B4 initial water](water.md), and [C1 drainage](drainage.md) extend it; the current build uses `basins-1` and protocol 7. Plate generation is unchanged. B1's measurements and recipe below are historical, not a description of geological time integration.
