...
[C2a basin analysis](basins.md) adds a Rust connectivity hierarchy, merge thresholds, and level–storage queries. [C2b inspection](basin-inspection.md) integrates its unchanged `basin-analysis-1` algorithm into generation, versioned binary transport, two map/globe layers, and a hierarchy inspector. [C3a](reservoir-experiment.md) and [C3b](reservoir-pair.md) supply reservoir experiments; [C3c](spill-connections.md) supplies geometric sill connections and passage queries. [C3d](nested-reservoir.md) implements closed binary nested filling for at most 128 regions with unique sill contacts, exclusive stocks, and checkpoints. These are standalone tools, not desktop/world dynamics. General multiway/alternative-outlet allocation, planetary inventory assignment, splitting, and erosion remain unimplemented.
...
Current implementation scope is milestone A, the native/GPU integration and diagnostic transport test, B1 plate kinematics, B2 initial crust, B3 explainable elevation, B4 initial water filling, B5 water-surface display, C1 drainage structure, C2a basin analysis, C2b basin inspection, C3a–C3b reservoir experiments, C3c geometric spill connections, and C3d bounded binary nested filling. Next specify multiway/alternative-outlet allocation and test pulse-order sensitivity before integrating planetary inventories. Remaining milestone B features follow as separate increments. Climate equations, economic modeling, and political entities must not block the first explainable terrain map.
