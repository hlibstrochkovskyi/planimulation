...
[C2a basin analysis](basins.md) adds a Rust connectivity hierarchy, merge thresholds, and level–storage queries. [C2b inspection](basin-inspection.md) integrates its unchanged `basin-analysis-1` algorithm into generation, versioned binary transport, two map/globe layers, and a hierarchy inspector. [C3a](reservoir-experiment.md) adds isolated-leaf storage and collector accounting; [C3b](reservoir-pair.md) adds closed-pair filling, spill, merging, and checkpoints. These are standalone experiments, not desktop/world dynamics. General nested/multiway routing, splitting, and erosion remain unimplemented.
...
Current implementation scope is milestone A, the native/GPU integration and diagnostic transport test, B1 plate kinematics, B2 initial crust, B3 explainable elevation, B4 initial water filling, B5 water-surface display, C1 drainage structure, C2a basin analysis, C2b basin inspection, and the standalone C3a–C3b reservoir experiments. Next specify and test receiving paths and conservative exchange in a controlled nested hierarchy before integrating planetary inventories. Remaining milestone B features follow as separate increments. Climate equations, economic modeling, and political entities must not block the first explainable terrain map.
